...
export default function CartItem({id,price,name,size,colour,image,qty,updateProductQty,deleteItem,updateItem}){
    console.log(qty)
    return(
        <section className="cart-item">
            ...
            <div className="cart-item-price-qty">
                <div className="cart-item-qty"><input type="number" value={qty??1} onChange={(e)=>updateProductQty(id,e)}/></div>
                <div className="cart-item-price"><p>{price}</p></div>
                <div className="cart-item-action">
                    <span onClick={updateItem}>Update</span>
                    <span onClick={()=>deleteItem(id)}>Delete</span>
                </div>
            </div>
        </section>
    )
